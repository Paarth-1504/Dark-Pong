...
class Block(pygame.sprite.Sprite):
	def __init__(self,path,x_pos,y_pos):
		...
		self.rect = self.image.get_rect(center = (int(x_pos),int(y_pos)))
...
class Ball(Block):
	...
	def restart_counter(self):
		...
		time_counter_rect = time_counter.get_rect(center = (int(screen_width/2),int(screen_height/2) + 50))
...
class GameManager:
	...
	def draw_score(self):
		...
		player_score_rect = player_score.get_rect(midleft = (int(screen_width / 2) + 40,int(screen_height/2)))
		opponent_score_rect = opponent_score.get_rect(midright = (int(screen_width / 2) - 40,int(screen_height/2)))
...
middle_strip = pygame.Rect(int(screen_width/2) - 2,0,4,int(screen_height))
# Game objects
